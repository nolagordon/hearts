...
public class TDLearner implements PlayerInterface {
	...
	public TDLearner(int numFeatures, boolean and, boolean or) {
		...
		if (and)
			totalFeatures += numFeatures * numFeatures;
		if (or)
			totalFeatures += numFeatures * numFeatures;
	...
	public void printWeights() {
		double[] weights = learner.getWeights();
	...
	public Card playTurn(Game game) {
		...
		// reach back into the map to find the card associated with the best
		// move
	...
	public int selectAction(ArrayList<GameState> states) {
		...
		// expected value
		// go with action with the least value (bc hearts is won by winning as
		// few pts as possible)
